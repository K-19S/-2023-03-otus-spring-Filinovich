...
import ru.otus.filinovich.dao.role.RoleRepository;
...
public class DatabaseChangelog {
    ...
    @ChangeSet(order = "006", id = "insertUserRoles", author = "s_filinovich")
    public void insertUserRoles(RoleRepository repository) {
        roles.add(new Role(ERole.ROLE_USER));
        roles.add(new Role(ERole.ROLE_ADMIN));
        roles = repository.saveAll(roles);
    }

    @ChangeSet(order = "007", id = "insertUsers", author = "s_filinovich")
    public void insertUsers(UserRepository repository) {
        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        User admin = new User("admin", "admin@admin.ru", encoder.encode("admin"));
        admin.getRoles().add(roles.get(1));
        users.add(admin);
        User user = new User("user", "user@user.ru", encoder.encode("user"));
        admin.getRoles().add(roles.get(1));
        users.add(user);
