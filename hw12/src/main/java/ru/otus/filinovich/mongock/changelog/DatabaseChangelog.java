...
import ru.otus.filinovich.domain.UserRole;
...
public class DatabaseChangelog {
    ...
    @ChangeSet(order = "006", id = "insertDefaultUsers", author = "s_filinovich")
    public void insertDefaultUsers(UserRepository userRepository) {
        List<User> users = new ArrayList<>();
        BCryptPasswordEncoder encoder = new BCryptPasswordEncoder();
        users.add(new User("admin", encoder.encode("admin"), Set.of(new UserRole("ADMIN"))));
        users.add(new User("user", encoder.encode("user"), Set.of(new UserRole("USER"))));
        users.add(new User("otus", encoder.encode("otus"), Set.of(new UserRole("USER"))));
